...
png('images/plot4.png', width=480, height=480, units='px')
# plot 4 - draft
par(mfrow=c(2,2))
plot(as.POSIXlt(df[,date_time],'%d/%m/%Y %H:%M:%S', tz='BRST'), df[,Global_active_power], xlab='', ylab='Global Active Power (kilowatts)', type='l')
plot(as.POSIXlt(df[,date_time],'%d/%m/%Y %H:%M:%S', tz='BRST'), df[,Voltage], type='l', ylab='Voltage', xlab='datetime')
plot(as.POSIXlt(df[,date_time],'%d/%m/%Y %H:%M:%S', tz='BRST'), df[,Sub_metering_1], xlab='', ylab='Energy sub metering', type='l', col='black')
lines(as.POSIXlt(df[,date_time],'%d/%m/%Y %H:%M:%S', tz='BRST'), df[,Sub_metering_2], col='red')
lines(as.POSIXlt(df[,date_time],'%d/%m/%Y %H:%M:%S', tz='BRST'), df[,Sub_metering_3], col='blue')
legend('topright', 
       legend=c('Sub_metering_1', 'Sub_metering_2', 'Sub_metering_3'), 
       col=c('black', 'red', 'blue'), 
       lty=1, bty='n')
plot(as.POSIXlt(df[,date_time],'%d/%m/%Y %H:%M:%S', tz='BRST'), df[,Global_reactive_power], type='l', ylab='Global_reactive_power', xlab='datetime')
# closes the device
dev.off()
